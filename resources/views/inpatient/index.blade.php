...
@section('page-title', 'Inpatient Registration')
@section('page-description', 'Register and manage inpatient admissions')
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-800">Inpatient Registrations</h2>
                    <a href="{{ route('inpatient.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition">
                        New Registration
                    </a>
                </div>
                ...
                <!-- Filters -->
                <form method="GET" action="{{ route('inpatient.index') }}" class="mb-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ward / Bed</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($registrations as $registration)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $registration->ward->name }}</div>
                                        <div class="text-sm text-gray-500">Bed {{ $registration->bed->bed_number }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex gap-2">
                                            <a href="{{ route('inpatient.show', $registration) }}" class="text-blue-600 hover:text-blue-900">View</a>
                                            @if($registration->status === 'active')
                                                <form action="{{ route('inpatient.discharge', $registration) }}" method="POST" onsubmit="return confirm('Discharge this patient?')">
                                                    @csrf
                                                    <button type="submit" class="text-red-600 hover:text-red-900">Discharge</button>
                                                </form>
                                            @endif
                                        </div>
                                    </td>
